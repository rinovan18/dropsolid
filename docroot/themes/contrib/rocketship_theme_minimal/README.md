...
Rocketship content types use Layout Builder to build up Sections (rows) and regions in 1 to 3 columns, to output node
content. For the homepage, different layout templates from Rocketship Core are used but you can make and add your own.
An example is provided in the theme/04-templates folder.
...
When using Rocketship (or Dropsolid) Features (not publicly available yet, as of this writing):

You will see that most Rocketship contenttypes have at least 2 view-modes they use:
- teaser: for the main overview
- full content: for the node detail content

When click the Layout Builder button on the full content node, you will see that different sections make up different parts of the node.
At the very least, you should see a section that contains the 'head' of your node and one for the 'content'. Both should have custom classes in order to better style them.
The naming follows the BEM rules for CSS.


## Features

Only important if using Rocketship (or Dropsolid) Features:

Important to know for theming, is that these create contenttypes which have view modes with layout templates and classes for you to theme them.
...
See the info in the 'development' readme of your Rocketship project, for making your own views, how to add them to the homepage or adding them to a Page.

### Details

As mentioned before, the Layout Builder page for this loads the entity content's
view mode 'full' or 'full content'.
...
If you have a rocketship theme enabled, each Feature and Paragraphs
loads styling from this theme, when the Dropsolid Feature of Paragraphs modules are enabled.

This is done by attaching a library via the theme's `rocketship_theme_minimal.theme` file's `HOOK_preprocess_page`

The styling can be changed in the `component/02-features` and `component/01-paragraphs` files
...
**Note:**
- Saving this form will also a default colors CSS-file that is loaded if you check the `Enable default CSS` options. Dropsolid sites DO NOT USE the default CSS, so you still need to add your front-facing colors and other theming in the `03-paragraphs` folder of your theme.
