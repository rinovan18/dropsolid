...
namespace Drupal\rocketship_blog_basic\Plugin\Block;
...
/**
 * Provides a 'FrontpageBlogView' block.
 *
 * @Block(
 *  id = "frontpage_blog_view",
 *  admin_label = @Translation("Frontpage Blog listing"),
 * )
 */
class FrontpageBlogView extends BlockBase implements ContainerFactoryPluginInterface {
  ...
  public function build() {
    ...
    $uuid = 'ab3aa739-180e-4432-9a19-1ea36a915d0b';
    ...
    if ($nodes) {
      ...
      $build['header'] = [
        '#type' => 'container',
        '#attributes' => ['class' => ['blog-front-header-wrapper']],
      ];
      $build['header']['title']['#markup'] = '<h2>' . $node->label() . '</h2>';
      $link = $node->toLink($this->t('View all blog items'))->toRenderable();
      $build['header']['link'] = $link;
    }

    $view_id = 'blog_overview';
    $view_display = 'blog_overview_front';
